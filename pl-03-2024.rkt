...
;;;  sin-repeticiones(l):
;;;
;;; 1. Base       : l es la lista vacia -> sin-repeticiones(()) = ()
;;; 2. Recurrencia: l no es la lista vacía; es decir, l=cons(car(l), cdr(l))
;;;      Hipótesis: se supone conocido sin-repeticiones(cdr(l))=H
;;;          Tesis: sin-repeticiones(l) = si car(l) está en cdr(l) entonces
;;;                                       H
;;;                                    si no
;;;                                       cons(car(l),H)
;;;
...
(define (sin-repeticiones l)
  (cond [(null? l) '()]
        [(member (car l) (cdr l)) (sin-repeticiones (cdr l))]
        [else (cons (car l) (sin-repeticiones (cdr l)))]))
